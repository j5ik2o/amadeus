...
|---|---|---|---|---|---|
| 3.1 | Functional Design | CONDITIONAL per Unit | 対象 Unit が Construction scope に含まれ、Functional Design が必要な場合 | `amadeus-construction` | `construction/<unit-name>/functional-design/`、`state.json` |
| 3.2 | Bolt Preparation | ALWAYS per Bolt | Inception gate が `passed` で対象 Bolt が確定している | `amadeus-construction-bolt-preparation` | `design.md`、`tasks.md`、`notes.md`、`traceability.md`、`state.json` |
| 3.3 | Implementation Execution | ALWAYS per ready Bolt | Design Gate が `ready` または `passed` で、Task が generated の場合 | `amadeus-construction-implementation-execution` | 実装コード、テストコード、`design.md`、`notes.md` |
...
| Lead Skill | `amadeus-construction` |
| Mode | public |
| Per-Unit | Yes |
...
7. `FunctionalDesignGateResult` は `status` から導出し、`state.json` には保存しない。

### Outputs

| Artifact | Description |
|---|---|
| `construction/<unit-name>/functional-design/business-logic-model.md` | 対象 Unit の業務ロジック構造 |
| `construction/<unit-name>/functional-design/business-rules.md` | 対象 Unit の業務ルール |
| `construction/<unit-name>/functional-design/domain-entities.md` | 対象 Unit の Domain Entity |
| `construction/<unit-name>/functional-design/frontend-components.md` | `frontendSurface: present` の場合だけ必要な UI 構成 |
| `state.json` | Functional Design の Unit 別状態 |

### Approval Gate

`ready_for_approval` は、Amadeus DLC Contract Catalog から導出した必須成果物の構造検証が通った場合だけ待機状態として扱う。

検証が失敗した場合は、`ready_for_approval` ではなく失敗した CheckResult として扱う。
...
`requirement` と `status` は任意に組み合わせない。
Validator は Amadeus DLC Contract Catalog の matrix から無効な組み合わせを `fail` の CheckResult にする。

### Notes

`state.json.construction.functionalDesign` は成果物要求を所有しない。
