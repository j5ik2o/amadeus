...
Amadeus Inception では、要求、ユースケース、Unit、Bolt をつなぐ。
Task は Construction で Bolt から導く。
...
Task は Construction Design を作成した後で、Bolt の実行範囲と Unit Design Brief から導く。
...
`units.md` の `詳細` は `units/<unit-id>-<slug>.md` を指す。
Unit のモジュールファイルは Unit の価値境界と要求対応を扱う入口である。
Unit のモジュールファイルから対応するモジュールディレクトリ配下の `design.md` へリンクする。

`bolts.md` は `設計` 列を持つ。
`設計` は、対象 Unit の `units/<unit-id>-<slug>/design.md` を指す。
複数 Unit をまたぐ Bolt では、複数の Unit Design Brief へリンクする。
その場合は Bolt のモジュールファイルの `複数 Unit を扱う理由` に複数 Unit をまたぐ理由を書く。
...
Unit のモジュールファイル、`requirements.md`、`use-cases.md` は、追跡や疑義解消のために参照できる。
...
`tasks.md` は Construction Design を根拠に生成する。

## 追跡

Inception の `traceability.md` の `設計からの追跡` は、Unit Design Brief から Bolt までの追跡を扱う。

```md
| 設計 | ユニット | 要求 | ユースケース | ボルト |
|---|---|---|---|---|
| [design.md](units/U001-example/design.md) | U001 | R001 | UC001 | B001 |
```
...
- `units.md` の `詳細` が `units/<unit-id>-<slug>.md` を指す。
- Unit のモジュールファイルに `関連成果物` 見出しがある。
- Unit のモジュールファイルの `関連成果物` からモジュールディレクトリ配下の `design.md` へリンクできる。
- Unit `design.md` に必須見出しがある。
- Unit `design.md` の各必須見出しに本文がある。
- `bolts.md` に `設計` 列がある。
- `bolts.md` の `ユニット` と `設計` リンク先の Unit ID が対応する。
- Bolt のモジュールファイルに `設計` 見出しがある。
- Bolt のモジュールファイルの `対象ユニット` と `設計` リンク先の Unit ID が対応する。
...
- Task 生成前の Construction Design は、対象 Bolt、Unit、Requirement を参照する。
- Task 生成後は、`Construction Design からの追跡` が Design Gate evidence と同じ `design.md` と対象 Bolt の全 Task を結ぶ。
- Construction 完了時は、対象 Bolt の `test-results.md` が必須成果物に含まれ、`Construction Design からの追跡` の `実装` と `検証` に `未実施` を残さない。
- `state.json.inception.requiredArtifacts` に Unit のモジュールファイル と Unit `design.md` が含まれる。
...
後方互換は残さない。
現在のあるべき姿から外れる `units/<unit-id>-<slug>/unit.md` と、Inception 旧 Bolt Design Brief としての `bolts/<bolt-id>-<slug>/design.md` は許容しない。
