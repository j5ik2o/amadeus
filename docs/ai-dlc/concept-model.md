...
Requirements は Units、Bolts、Tasks を所有しない。

Requirement から Unit、Bolt、Task を見たい場合は、Requirement を参照している成果物を逆引きする projection として扱う。
...
Requirement、Story、Unit、Bolt、Task を ID で接続し、参照関係と検証状態を記録する。

## 参照関係

Requirement は要求の定義元である。

Unit、Bolt、Task は Requirement を参照する。
...
Requirement は Unit、Bolt、Task を知らない。
