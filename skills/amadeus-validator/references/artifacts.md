...
- `construction.bolts[]` は対象 Bolt の `id`、`designGate`、`taskPlan` を持つ。
...
- `construction.bolts[].taskPlan.status` は `not_generated`、`generated`、`blocked` のいずれかである。
- `construction.bolts[].taskPlan.reviewedBy` は空欄にしない。
- `construction.bolts[].taskPlan.updatedAt` は空欄にしない。
- `construction.bolts[].taskPlan.evidence` は対象 Bolt の `bolts/<bolt-id>-<slug>/tasks.md` を指す。
