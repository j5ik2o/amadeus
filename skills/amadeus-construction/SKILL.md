...
5. 対象 Bolt の `design.md` に必須見出しがある。
6. 対象 Bolt の `tasks.md` に作業、要求、ユースケース、依存、設計根拠、証拠がある。
7. `state.json.construction.bolts[]` が対象 Bolt の Design Gate、Tasks 生成状態、evidence を持つ。
8. `traceability.md` に `Construction Design からの追跡` があり、Design から Task まで追跡できる。
