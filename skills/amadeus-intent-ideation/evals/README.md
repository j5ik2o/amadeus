...
| `scaffold-only` | 完了 | Ideation 成果物だけを作り、Intent Validator が対象 Intent ディレクトリ名指定で pass する。要求、ユースケース、Unit、Bolt、domain は作らない。 | `/tmp/amadeus-intent-ideation-scaffold-only.4xHUf4` |
| `repair` | 完了 | 既存 Ideation 成果物の構造だけを補修し、Intent Validator が対象 Intent ディレクトリ名指定で pass する。要求、ユースケース、Unit、Bolt、domain は作らない。 | `/tmp/amadeus-intent-ideation-repair.Nj7jRc` |
